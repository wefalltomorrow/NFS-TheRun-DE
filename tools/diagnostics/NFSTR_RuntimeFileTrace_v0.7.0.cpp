...
// NFSTR_RuntimeFileTrace.asi v0.7.0
// Startup-safe file I/O trace for Need for Speed: The Run v1.1 x86.
//
// Purpose:
//   Determine whether the PC runtime actually consumes the reconstructed
//   NFSGame patch layer during Escape From S.F. loading, while continuing
//   to trace San Francisco level files for correlation.
//
// Safety:
//   Hooks only the MAIN EXE import table. System/runtime DLLs and other ASIs
//   are never modified. The hooks are installed automatically from the worker
//   thread after the log file is opened; no F6 is required for this tracer.
...
static TargetKind ClassifyTarget(const std::wstring& p) {
    const std::wstring q = Lower(p);
    if (q.find(L"level_0100_sanfrancisco") != std::wstring::npos)
        return TargetKind::SanFrancisco;
    if (q.find(L"gameconfigurations\\nfsgame") != std::wstring::npos ||
        q.find(L"\\nfsgame\\nfsgame.") != std::wstring::npos)
        return TargetKind::NFSGame;
    return TargetKind::None;
}
static const char* KindTag(TargetKind k) {
    switch (k) {
        case TargetKind::SanFrancisco: return "SF";
        case TargetKind::NFSGame: return "NFSGAME";
...
static HANDLE WINAPI H_CreateFileW(LPCWSTR n,DWORD a,DWORD s,LPSECURITY_ATTRIBUTES sa,DWORD c,DWORD f,HANDLE t) {
    ...
    TargetKind kind = ClassifyTarget(p);
    if (h != INVALID_HANDLE_VALUE && kind != TargetKind::None) {
        FileState st; st.path=p; st.pos=0; st.known=true; st.kind=kind; PutFile(h,st);
        std::string cat = std::string(KindTag(kind)) + "-FILE";
        Log(cat.c_str(), true, "OPEN h=0x%p path=\"%s\" access=0x%08lX caller=%s",
...
static BOOL WINAPI H_GetFileSizeEx(HANDLE h, PLARGE_INTEGER sz) {
    ...
    if (!st.path.empty())
        Log((std::string(KindTag(st.kind))+"-SIZE").c_str(), true, "h=0x%p size=%lld result=%d error=%lu caller=%s",
...
static BOOL WINAPI H_SetFilePointerEx(HANDLE h,LARGE_INTEGER d,PLARGE_INTEGER np,DWORD method) {
    ...
    if (!st.path.empty()) {
        ...
        Log((std::string(KindTag(st.kind))+"-SEEK").c_str(), true, "h=0x%p move=%lld method=%lu result=%d new=%s%llu error=%lu caller=%s",
...
static DWORD WINAPI H_SetFilePointer(HANDLE h,LONG lo,PLONG hi,DWORD method) {
    ...
    if (!st.path.empty()) {
        ...
        Log((std::string(KindTag(st.kind))+"-SEEK").c_str(), true, "h=0x%p moveLo=%ld method=%lu result=0x%08lX new=%llu error=%lu caller=%s",
...
static BOOL WINAPI H_ReadFile(HANDLE h,LPVOID b,DWORD want,LPDWORD got,LPOVERLAPPED ov) {
    ...
    if (!st.path.empty())
        Log((std::string(KindTag(st.kind))+"-READ").c_str(), true, "#%ld BEGIN h=0x%p off=%s%llu want=%lu overlapped=%d caller=%s",
    ...
    DWORD n = got ? *got : 0;
    if (!st.path.empty()) {
        Log((std::string(KindTag(st.kind))+"-READ").c_str(), true, "#%ld END result=%d got=%lu error=%lu",(long)id,r,(unsigned long)n,(unsigned long)e);
...
static BOOL WINAPI H_CloseHandle(HANDLE h) {
    FileState st = GetFile(h);
    if (!st.path.empty())
        Log((std::string(KindTag(st.kind))+"-FILE").c_str(), true, "CLOSE h=0x%p path=\"%s\" caller=%s",
...
static DWORD WINAPI Worker(LPVOID) {
    ...
    if (slash) *(slash+1)=0;
    wchar_t lp[MAX_PATH]{};
    _snwprintf(lp,MAX_PATH-1,L"%sNFSTR_RuntimeFileTrace.log",me);
    ...
    Log("INIT",true,"NFSTR Runtime File Trace v0.7.0 loaded PID=%lu EXEbase=0x%p",
        (unsigned long)GetCurrentProcessId(),GetModuleHandleW(nullptr));
    Log("INIT",true,"AUTO TRACE: targets=NFSGame+SanFrancisco; MAIN EXE IAT only; no F6 required.");

    int n=ArmHooks();
    if (n>0) {
        Log("ARM",true,"AUTO ARMED: patched %d API imports in MAIN EXE only",n);
    } else {
        Log("ARM",true,"AUTO ARM FAILED: no matching imports were patched");
    }

    while (WaitForSingleObject(g_stop,50)==WAIT_TIMEOUT) {
        // Passive wait. File hooks do all logging.
    }
